...
void print_vec_acc(std::vector<double> &vec,bool vert,int size)
{
    ...
    if (not vert)
    {		
    	for (int i=0;i<size;i++){cout<<boost::format("%+.15f") %vec[i]<<"  ";}
    	cout<<endl;
    }
    else
    {   for (int i=0;i<size;i++){cout<<boost::format("%+.15f") %vec[i]<<endl;} }
}

void print_vec_scientific(std::vector<double> &vec,bool vert,int size)
{
    if (size==0) {size=vec.size();}
...
void print_real_mat(Matrix &mat)
{
    ...
    for (i=0;i<mat.NRows();i++)
    {
        for (j=0;j<mat.NCols();j++) cout<<boost::format("%+.5f") % mat(i,j)<<" ";
        cout<<endl;
    }
    cout<<endl;
}
